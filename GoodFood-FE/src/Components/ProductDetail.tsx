...
const ProductDetail = () => {
    ...
    const fetchDetail = async (filter: string, pageNum: number) => {
        try {
            ...
            setProductImgs(() => {
                const imgs: ProductImages[] = response.data.data.productImages || [];
                const cover = response.data.data.product.coverImage;

                return imgs.some(img => img.image === cover)
                    ? imgs
                    : [{ productImageID: -1, image: cover, productID: response.data.data.product.productID }, ...imgs];
            });
                                        ...
                                        <div>
                                            <div className="salary" >
                                                ...
                                                <div className="box-add-salary">
                                                    ...
                                                    <input type="text" className="form-control" value={quantity} name="soLuong" min={1} max={5} style={{ width: '50px' }} id="soLuong" readOnly={true} />
                                                    <button type="button" className="btn btn-sm" onClick={() => { changeQuantity(1) }} >+</button>
                                                </div>
                                            </div>
